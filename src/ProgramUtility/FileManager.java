    ...
    public void writeCollection(){
        if (fileName != null) {
            try {
                ...
                System.out.println(getLastSaveTime());
            } catch (JAXBException e){
                Console.printError("The collection has encountered an error");
            } catch (IOException e) {
                Console.printError("File can't be opened!");
            }
        } else Console.printError("File not found!");
    }
    ...
    public LabCollection readCollection() {
        if (fileName != null) {
            try {
                ...
                Console.printError("XML file structure error. File can't be read!");
                System.exit(0);
            }
        } else Console.printError("File not found!");
        return new LabCollection(new LinkedList<>());
